...
def generate_html_report(excel_path: str, html_path: str):
    ...
    legacy_lookup_df = sheets.get("Legacy Ratings Lookup", pd.DataFrame())
    ...
    legacy_lookup_json = _safe_json(legacy_lookup_df)

    # Get unique values for filters
    entities = sorted(audit_df["Entity ID"].unique().tolist()) if "Entity ID" in audit_df.columns else []
    # Build entity ID -> name mapping for dropdown display
    entity_name_map = {}
    if "Entity ID" in audit_df.columns and "Entity Name" in audit_df.columns:
        for _, row in audit_df[["Entity ID", "Entity Name"]].drop_duplicates().iterrows():
            eid = str(row["Entity ID"])
            ename = str(row["Entity Name"]) if pd.notna(row["Entity Name"]) else ""
...
<div class="sidebar" id="sidebar">
    ...
    <div class="view-radio">
        ...
        <label><input type="radio" name="view" value="portfolio" onchange="switchView(this.value)"> Portfolio Overview</label>
    </div>
    <div class="divider"></div>
...
<div id="tab-entity" class="tab-content active">
    ...
    <div id="entity-tab-legacy" class="sub-tab-content">
        <div id="entity-legacy-profile"></div>
    </div>
</div>
...
const legacyLookupData = {legacy_lookup_json};
const entities = {json.dumps(entities)};
const entityNames = {json.dumps(entity_name_map)};
...
function switchEntityTab(name) {{
    ...
    let idx = ["profile","drill","trace","source","legacy"].indexOf(name);
...
function renderPortfolio() {{
    ...
    let assumedNA = data.filter(r => r["Status"] === "No Evidence Found \\u2014 Verify N/A").length;
    ...
    let notAssessed = data.filter(r => r["Status"] === "Not Assessed").length;
...
function renderEntityView() {{
    ...
    // --- Legacy Profile tab ---
    let legacyHtml = "";
    if (legacyLookupData.length) {{
        let eidCol = legacyLookupData[0].hasOwnProperty("Entity ID") ? "Entity ID" : (legacyLookupData[0].hasOwnProperty("Audit Entity ID") ? "Audit Entity ID" : null);
        if (eidCol) {{
            let el = legacyLookupData.filter(l => String(l[eidCol]||"").trim() === eid);
            if (el.length) {{
                legacyHtml += '<h3>Legacy Pillar Ratings</h3>';
                let cols = Object.keys(el[0]).filter(c => c !== eidCol);
                legacyHtml += '<div class="table-wrap"><table class="md-table"><thead><tr>' + cols.map(c => `<th>${{esc(c)}}</th>`).join("") + '</tr></thead><tbody>';
                el.forEach(r => {{ legacyHtml += '<tr>' + cols.map(c => `<td>${{esc(String(r[c]||""))}}</td>`).join("") + '</tr>'; }});
    ...
    }} else {{ legacyHtml = '<p class="meta">No Legacy Ratings Lookup data in workbook.</p>'; }}
    document.getElementById("entity-legacy-profile").innerHTML = legacyHtml;
}}
...
function renderRiskView() {{
    ...
    let maxCount = Math.max(...Object.values(ratingCounts), 1);
    let barHtml = '<div style="max-width:500px;">';
    Object.keys(ratingCounts).forEach(label => {{
        let ct = ratingCounts[label];
        if (ct === 0) return;
        let pct = (ct / maxCount * 100).toFixed(0);
        let color = chartColors[label] || "#ccc";
        barHtml += `<div style="margin:6px 0;display:flex;align-items:center;gap:10px;">
            <span style="min-width:120px;font-size:13px;text-align:right;">${{label}}</span>
            <div style="flex:1;background:#e9ecef;border-radius:4px;height:24px;">
                <div style="width:${{pct}}%;background:${{color}};height:100%;border-radius:4px;min-width:2px;"></div>
            </div>
            <span style="min-width:30px;font-size:13px;font-weight:600;">${{ct}}</span>
        </div>`;
    }});
    barHtml += '</div>';
...
window.addEventListener("load", () => {{
    ...
    entities.forEach(e => {{ let o = document.createElement("option"); o.value = e; o.text = e + ' - ' + (entityNames[e] || ''); eSelect.add(o); }});
